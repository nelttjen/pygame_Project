...
class BaseController:
    ...
    def __init__(self, boat, level):
        ...
        boat.radarCallbacks[Config.Collisiontypes.BOAT].append(
            self.update_boat_sensors)
        boat.radarCallbacks[Config.Collisiontypes.SHORE].append(
            self.update_shore_sensors)
        self.count = -1
    ...
    def update(self):
        ...
        self.orientationA = self.body.rotation_vector.get_angle_between(
            self.dxy)
        ...
        if Config.Screen.DEBUG:
            self.STDSCR.addstr(
                self.scrline + 0, 0, f'X:{x:.2f} Y:{y:.2f} GoalCP: {self.next_checkpoint} '
                f'GoalX:{self.boat.next_checkpoint_x:.2f} '
                f'GoalY:{self.boat.next_checkpoint_y:.2f} '
                f'Force:{self.move:.2f} Turn:{self.turn:.2f}', curses.color_pair(1))
            self.STDSCR.addstr(
                self.scrline + 1, 0, f'OrientationA:{self.orientationA:.2f} '
